...
# ==============================================================================
# [2] 벽사다리/용마루 시스템 관련 엑셀 및 그리기 함수
# ==============================================================================
def create_ladder_excel_bytes(data):
...
def draw_pipe_ladder(ax, x1, y1, x2, y2, t, zorder=1, facecolor='white'):
    dx = x2 - x1
    dy = y2 - y1
...
def draw_diag_poly_ladder(ax, x_start_tip, x_end_tip, y_bot, y_top, w_half, is_forward, zorder=1, facecolor='yellow'):
    t_h = 2 * w_half  
    if is_forward: 
        pts = [[x_start_tip, y_bot], [x_start_tip + t_h, y_bot], [x_end_tip, y_top], [x_end_tip - t_h, y_top]]
    else:          
        pts = [[x_start_tip, y_top], [x_start_tip + t_h, y_top], [x_end_tip, y_bot], [x_end_tip - t_h, y_bot]]
...
# 메인 스트림릿 앱 UI
# ==============================================================================
st.title("🏢 하나천막기업 자재 산출 및 도면 시스템")

tab1, tab2 = st.tabs(["맞춤형 트러스 생성기", "벽/보강 사다리 산출"])

# ==================== TAB 1: 트러스 ====================
with tab1:
    st.header("맞춤형 트러스(밑더블/아치 등) 생성")
    ...
    with col1:
        ...
        type_choice = st.selectbox("트러스 형태 선택", options=list(type_options.keys()), format_func=lambda x: type_options[x], index=6)
    ...
    with col2:
        ...
        if is_sub_type and type_choice == "5":
            h_outer_cm = st.number_input("양쪽 끝단(시작) 높이(cm)", value=51.0, step=1.0)
        elif is_sub_type:
            h_outer_cm = st.number_input("상하 일정 수직 폭(깊이)(cm)", value=80.0, step=1.0)
        elif is_double_bot:
            h_outer_cm = st.number_input("밑더블 양끝 외경 시작높이(cm)", value=80.0, step=1.0)
        else:
            h_outer_cm = st.number_input("끝단(시작) 높이(cm)", value=51.0, step=1.0)
            
        h_tie_cm = 0
        if has_tie:
            h_tie_cm = st.number_input("수평재(수평선) 바닥기준 높이(cm)", value=150.0, step=10.0)
            
        h_center_cm = st.number_input("최고점 상단 높이(cm)", value=250.0, step=10.0)

    with col3:
        m_od = st.number_input("상/하현부 및 수평 파이프 지름(mm)", value=59.9, step=1.0)
        v_od = st.number_input("다대(일반) 지름(mm)", value=38.1, step=1.0)
        r_od = st.number_input("용마루(중앙) 지름(mm)", value=59.9, step=1.0)
        d_od = st.number_input("살대(대각) 지름(mm)", value=31.8, step=1.0)
        offset_mm = st.number_input("살대 이격 거리(mm)", value=20.0, step=1.0)

    if st.button("트러스 도면 및 엑셀 생성", type="primary"):
        with st.spinner('도면과 데이터를 생성 중입니다...'):
            S, H_out, H_cen, H_tie = span_cm * 10, h_outer_cm * 10, h_center_cm * 10, h_tie_cm * 10
            t_name = type_options[type_choice]
            ...
            raw_data.append({"구분": "상현대(전체)", "품명": f"{m_od}mm 파이프", "재단기장(L)": round(total_top_chord_len, 1), "상단 가공각(°)": 0, "하단 가공각(°)": 0})
            raw_data.append({"구분": "하현대(전체)", "품명": f"{m_od}mm 파이프", "재단기장(L)": round(total_bot_chord_len, 1), "상단 가공각(°)": 0, "하단 가공각(°)": 0})
            ...
            if is_double_bot:
                ...
                raw_data.append({"구분": "밑더블수평재", "품명": f"{m_od}mm 파이프", "재단기장(L)": round(total_mid_pipe_len, 1), "상단 가공각(°)": 0, "하단 가공각(°)": 0})
            ...
            for i in range(divs + 1):
                ...
                if is_double_bot:
                    ...
                    if u_len > 0:
                        ...
                        raw_data.append({"구분": g_name, "품명": f"{curr_v_od}mm 파이프", "재단기장(L)": round(u_cut_max, 1), "상단 가공각(°)": t_angle, "하단 가공각(°)": 0})
                    ...
                    if l_len > 0:
                        ...
                        raw_data.append({"구분": g_name, "품명": f"{curr_v_od}mm 파이프", "재단기장(L)": round(l_cut_max, 1), "상단 가공각(°)": 0, "하단 가공각(°)": b_angle})
                    ...
                    raw_data.append({"구분": v_gubun, "품명": f"{curr_v_od}mm 파이프", "재단기장(L)": round(v_cut_l, 1), "상단 가공각(°)": t_angle, "하단 가공각(°)": b_angle})
                ...
                if is_diag:
                    ...
                    def draw_diag(left_x, right_x, is_forward):
                        ...
                        if is_forward:
                            ...
                            pts = [[x_bl, get_chord_y_bot(x_bl)], [x_br, get_chord_y_bot(x_br)], [x_tr, get_chord_y_top(x_tr)], [x_tl, get_chord_y_top(x_tl)]]
                        else:
                            x_br, x_bl = px_bot + w_half, px_bot - w_half
                            x_tl, x_tr = px_top - w_half, px_top + w_half
                            pts = [[x_br, get_chord_y_bot(x_br)], [x_bl, get_chord_y_bot(x_bl)], [x_tl, get_chord_y_top(x_tl)], [x_tr, get_chord_y_top(x_tr)]]
                            
                        ax.add_patch(plt.Polygon(pts, facecolor='#f1c40f', edgecolor='black', linewidth=1.2, zorder=3))
                        ...
                        raw_data.append({"구분": "살대", "품명": f"{d_od}mm 파이프", "재단기장(L)": round(diag_l, 1), "상단 가공각(°)": d_top_angle, "하단 가공각(°)": d_bot_angle})
                    ...
                    if is_double_bot:
                        def draw_custom_diag(left_x, right_x, gubun_name, is_forward):
                            ...
                            raw_data.append({"구분": gubun_name, "품명": f"{d_od}mm 파이프", "재단기장(L)": round(diag_l, 1), "상단 가공각(°)": d_top_angle, "하단 가공각(°)": d_bot_angle})
            ...
            if has_tie and H_tie > 0:
                ...
                if tie_length_inner > 0:
                    ...
                    raw_data.append({"구분": "수평재", "품명": f"{m_od}mm 파이프", "재단기장(L)": round(tie_length_inner, 1), "상단 가공각(°)": 0, "하단 가공각(°)": 0})
                ...
                for i, x in valid_inner_xs:
                    ...
                    x_l, x_r = x - curr_v_od / 2, x + curr_v_od / 2
                    yt_l, yt_r = get_chord_y_bot(x_l), get_chord_y_bot(x_r)
                    
                    if x_l < S/2 < x_r and not is_arch:
                        poly_top_inner = [[x_r, yt_r], [S/2, get_chord_y_bot(S/2)], [x_l, yt_l]]
                    else:
                    ...
                    raw_data.append({"구분": "수평내부다대", "품명": f"{curr_v_od}mm 파이프", "재단기장(L)": round(v_len, 1), "상단 가공각(°)": t_angle, "하단 가공각(°)": 0})
                ...
                for i in range(len(inner_intervals)-1):
                    ...
                    if wx_end > wx_start + 10: 
                        ...
                        dy_est = get_chord_y_bot(wx_start) - y_bot_limit if not is_forward_tie else get_chord_y_bot(wx_end) - y_bot_limit
                        ...
                        py_bot = y_bot_limit
                        py_top = get_chord_y_bot(px_top) 
                        diag_l = math.hypot(px_top - px_bot, py_top - py_bot)
                        
                        if is_forward_tie:
                            x_bl, x_br = wx_start, wx_start + 2*w_half_tie
                            x_tr, x_tl = wx_end, wx_end - 2*w_half_tie
                            pts = [[x_bl, y_bot_limit], [x_br, y_bot_limit], [x_tr, get_chord_y_bot(x_tr)], [x_tl, get_chord_y_bot(x_tl)]]
                        else:
                            x_br, x_bl = wx_end, wx_end - 2*w_half_tie
                            x_tl, x_tr = wx_start, wx_start + 2*w_half_tie
                            pts = [[x_br, y_bot_limit], [x_bl, y_bot_limit], [x_tl, get_chord_y_bot(x_tl)], [x_tr, get_chord_y_bot(x_tr)]]
                        ...
                        raw_data.append({"구분": "수평내부살대", "품명": f"{d_od}mm 파이프", "재단기장(L)": round(diag_l, 1), "상단 가공각(°)": d_top_angle, "하단 가공각(°)": d_bot_angle})
            ...
            with dl_col1:
                st.download_button(label="📄 PDF 도면 다운로드", data=pdf_buffer, file_name=f"Truss_{t_name}_{int(span_cm)}.pdf", mime="application/pdf")
            with dl_col2:
                st.download_button(label="📊 엑셀 재단표 다운로드", data=excel_bytes, file_name=f"Truss_{t_name}_{int(span_cm)}_재단표.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
# ==================== TAB 2: 벽/보강 사다리 산출 ====================
with tab2:
    st.header("벽사다리 및 보강사다리 통합 산출 시스템")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        st.subheader("기본 치수 입력")
        L_cm = st.number_input("전체 총기장(cm)", value=2000.0, step=10.0)
        W_cm = st.number_input("보강사다리 폭(cm)", value=70.0, step=1.0)
        H_truss_cm = st.number_input("메인사다리 높이(폭)(cm)", value=70.0, step=1.0)
        ...
        offset_mm_ladder = st.number_input("살대(대각재) 다대 꼭지점 이격 거리(mm)", value=10.0, step=1.0)
        offset_cm = offset_mm_ladder / 10.0

    with col2:
        st.subheader("용마루 & 벽사다리 치수")
        H_ridge_cm = st.number_input("용마루 폭(높이)(cm)", value=70.0, step=1.0)
        ridge_deduct_mm = st.number_input("용마루 공제(연결 파이프) 기준 사이즈(mm)", value=59.9, step=0.1)
        ridge_deduct_cm = ridge_deduct_mm / 10.0
        total_sets_ridge = st.number_input("용마루 전체 라인 제작 수량(세트)", value=1, step=1)
        wall_snagi_mm = st.number_input("벽사다리 스나기 사이즈(mm)", value=89.1, step=0.1)
        wall_snagi_cm = wall_snagi_mm / 10.0

    with col3:
        st.subheader("파이프 규격(mm) 설정")
        p_sub_main = st.number_input("보강사다리 상하현재 지름(mm)", value=38.1, step=0.1)
        p_sub_sub = st.number_input("보강사다리 수직/사재 지름(mm)", value=31.8, step=0.1)
        p_main_main = st.number_input("메인사다리 상하현재 지름(mm)", value=42.2, step=0.1)
        p_main_snagi = st.number_input("메인사다리 스나기 지름(mm)", value=89.1, step=0.1)
        p_main_v = st.number_input("메인사다리 수직다대 지름(mm)", value=38.1, step=0.1)
        p_main_diag = st.number_input("메인사다리 사다리살대 지름(mm)", value=31.8, step=0.1)
        p_ridge_main = st.number_input("용마루 상하현재 지름(mm)", value=42.2, step=0.1)
        p_ridge_v = st.number_input("용마루 수직다대 지름(mm)", value=38.1, step=0.1)
        p_ridge_diag = st.number_input("용마루 사다리살대 지름(mm)", value=31.8, step=0.1)

    if st.button("사다리 도면 및 엑셀 생성", type="primary"):
        with st.spinner('사다리 도면과 데이터를 생성 중입니다...'):
            max_span_main = 380.0 
            
            t_sub_main_cm, t_sub_sub_cm = p_sub_main / 10.0, p_sub_sub / 10.0
            t_main_main_cm, t_main_snagi_cm = p_main_main / 10.0, p_main_snagi / 10.0
            t_main_v_cm, t_main_diag_cm = p_main_v / 10.0, p_main_diag / 10.0
            t_ridge_main_cm, t_ridge_v_cm, t_ridge_diag_cm = p_ridge_main / 10.0, p_ridge_v / 10.0, p_ridge_diag / 10.0
            ...
            def calc_diag_ladder(spacing, v_len, left_r, right_r, t_diag):
            ...
            actual_sub_diag_len, sub_cut_angle, angle_s_deg, w_half_s = calc_diag_ladder(gap_s, actual_sub_v_len, t_sub_sub_cm/2, t_sub_sub_cm/2, t_sub_sub_cm)

            diag_len_snagi, cut_angle_snagi, angle_m_snagi_deg, w_half_m_snagi = calc_diag_ladder(sub_gap, actual_main_v_len, t_main_snagi_cm/2, t_main_v_cm/2, t_main_diag_cm)
            diag_len_norm, cut_angle_norm, angle_m_norm_deg, w_half_m_norm = calc_diag_ladder(sub_gap, actual_main_v_len, t_main_v_cm/2, t_main_v_cm/2, t_main_diag_cm)
            ...
            diag_norm, cut_norm, _, w_half_r_norm = calc_diag_ladder(S_norm, actual_ridge_v_len, t_ridge_v_cm/2, t_ridge_v_cm/2, t_ridge_diag_cm)
            diag_mid_c, cut_mid_c, _, w_half_r_mid = calc_diag_ladder(S_mid_deep, actual_ridge_v_len, ridge_deduct_cm/2, t_ridge_v_cm/2, t_ridge_diag_cm)
            diag_end_c, cut_end_c, _, w_half_r_end = calc_diag_ladder(S_end_deep, actual_ridge_v_len, ridge_deduct_cm/2, ridge_deduct_cm/2, t_ridge_diag_cm)
            ...
            if sub_div > 2:
                ...
                data.append([f"[메인사다리] 살대-일반({int(round(cut_angle_norm, 0))}°)", f"{p_main_diag}mm", qty_norm_diag, round(diag_len_norm,1), round((diag_len_norm*qty_norm_diag), 1), get_6m_count(diag_len_norm*qty_norm_diag)])
            data.append(["------------------", "----------", "---", "---", "---", "---"])
            ...
            draw_pipe_ladder(ax1, 0, y_top_center_s, L_cm, y_top_center_s, t_sub_main_cm, zorder=3, facecolor='#C0C0C0')
            draw_pipe_ladder(ax1, 0, y_bot_center_s, L_cm, y_bot_center_s, t_sub_main_cm, zorder=3, facecolor='#C0C0C0')
            
            v_centers_s = [(t_sub_sub_cm / 2) + i * gap_s for i in range(n_sec_s + 1)]
            for i in range(n_sec_s + 1):
                x = v_centers_s[i]
                draw_pipe_ladder(ax1, x, y_inner_bot_s, x, y_inner_top_s, t_sub_sub_cm, zorder=2, facecolor='#000080')
                if i < n_sec_s:
                    ...
                    if i % 2 == 0: 
                        draw_diag_poly_ladder(ax1, start_tip_s, end_tip_s, y_inner_bot_s, y_inner_top_s, w_half_s, True, zorder=1, facecolor='yellow')
                        ...
                        draw_diag_poly_ladder(ax1, start_tip_s, end_tip_s, y_inner_bot_s, y_inner_top_s, w_half_s, False, zorder=1, facecolor='yellow')
                        ax1.text((start_tip_s + end_tip_s)/2, W_cm/2, f"{round(actual_sub_diag_len,1)}cm\n({int(round(sub_cut_angle, 0))}°)", ha='center', va='center', fontsize=18, color='blue', rotation=-angle_s_deg) 
            ...
            draw_pipe_ladder(ax2, 0, y_top_center_m, L_cm, y_top_center_m, t_main_main_cm, zorder=3, facecolor='#C0C0C0')
            draw_pipe_ladder(ax2, 0, y_bot_center_m, L_cm, y_bot_center_m, t_main_main_cm, zorder=3, facecolor='#C0C0C0')
            
            for i in range(n_sec_m + 1):
                cx = i * gap_m
                draw_pipe_ladder(ax2, cx, -30, cx, H_truss_cm, t_main_snagi_cm, zorder=5, facecolor='purple')
                if i == 0:
                ...
                if i < n_sec_m:
                    ...
                    for j in range(1, sub_div + 1):
                        ...
                        if j < sub_div:
                            draw_pipe_ladder(ax2, curx, y_inner_bot_m, curx, y_inner_top_m, t_main_v_cm, zorder=4, facecolor='#000080')
                        ...
                        if j % 2 != 0:
                            draw_diag_poly_ladder(ax2, start_tip_m, end_tip_m, y_inner_bot_m, y_inner_top_m, w_half, True, zorder=3, facecolor='yellow')
                            ...
                            draw_diag_poly_ladder(ax2, start_tip_m, end_tip_m, y_inner_bot_m, y_inner_top_m, w_half, False, zorder=3, facecolor='yellow')
                            ax2.text((start_tip_m + end_tip_m)/2, H_truss_cm/2, f"{round(curr_diag_len,1)}cm\n({int(round(curr_cut_angle, 0))}°)", ha='center', va='center', fontsize=18, color='blue', rotation=-curr_angle_deg) 

            for i in range(n_sec_m):
                span_center = i * gap_m + (gap_m / 2)
                if n_sec_m == 1: wall_len, w_label = L_wall_single, "단일"
                elif i == 0 or i == n_sec_m - 1: wall_len, w_label = L_wall_end, "양끝"
                else: wall_len, w_label = L_wall_mid, "중간"
                ax2.text(span_center, -65, f"▼ 벽사다리({w_label}) 가로 절단(스나기 {wall_snagi_mm}mm): {round(wall_len, 1)}cm", ha='center', color='darkgreen', weight='bold', fontsize=22) 
            ...
            for i in range(n_sec_m + 1):
                ...
                draw_pipe_ladder(ax3, snagi_cx, -30, snagi_cx, H_ridge_cm, ridge_deduct_cm, zorder=5, facecolor='purple')

                if i < n_sec_m:
                    cx_start = i * gap_m
                    if n_sec_m == 1:
                        x_min, x_max = ridge_deduct_cm, L_cm - ridge_deduct_cm
                        label = "[단일세트]"
                    elif i == 0:
                        x_min, x_max = ridge_deduct_cm, gap_m - ridge_deduct_cm / 2
                        label = "[양끝세트]"
                    elif i == n_sec_m - 1:
                        x_min, x_max = cx_start + ridge_deduct_cm / 2, L_cm - ridge_deduct_cm
                        label = "[양끝세트]"
                    else:
                        x_min, x_max = cx_start + ridge_deduct_cm / 2, (i+1)*gap_m - ridge_deduct_cm / 2
                        label = "[중간세트]"
                    ...
                    draw_pipe_ladder(ax3, x_min, y_top_center_r, x_max, y_top_center_r, t_ridge_main_cm, zorder=3, facecolor='#C0C0C0')
                    draw_pipe_ladder(ax3, x_min, y_bot_center_r, x_max, y_bot_center_r, t_ridge_main_cm, zorder=3, facecolor='#C0C0C0')

                    for vx in v_centers:
                        draw_pipe_ladder(ax3, vx, y_inner_bot_r, vx, y_inner_top_r, t_ridge_v_cm, zorder=4, facecolor='#000080')

                    for j in range(4):
                        span_dist = v_centers[j+1] - v_centers[j]
                        is_left_snagi, is_right_snagi = (j == 0), (j == 3)
                        l_r = ridge_deduct_cm/2 if is_left_snagi else t_ridge_v_cm/2
                        r_r = ridge_deduct_cm/2 if is_right_snagi else t_ridge_v_cm/2
                        
                        diag_len, cut_angle, angle_rad, w_half_r = calc_diag_ladder(span_dist, actual_ridge_v_len, l_r, r_r, t_ridge_diag_cm)
                        ...
                        ax3.text(mid_span_x, -H_ridge_cm*0.15, f"자간:\n{round(span_dist,1)}cm", ha='center', fontsize=18, color=txt_color, weight='bold') 

                        if j % 2 == 0:
                            draw_diag_poly_ladder(ax3, start_tip_r, end_tip_r, y_inner_bot_r, y_inner_top_r, w_half_r, True, zorder=3, facecolor='yellow')
                            ...
                            draw_diag_poly_ladder(ax3, start_tip_r, end_tip_r, y_inner_bot_r, y_inner_top_r, w_half_r, False, zorder=3, facecolor='yellow')
                            ax3.text((start_tip_r + end_tip_r)/2, H_ridge_cm/2, f"{round(diag_len,1)}\n({int(round(cut_angle, 0))}°)", ha='center', va='center', fontsize=16, color='blue', rotation=-angle_rad) 
            ...
            dl_col1, dl_col2 = st.columns(2)
            with dl_col1:
                st.download_button(label="📄 PDF 도면 다운로드", data=pdf_buffer_ladder, file_name=f"사다리도면_{int(L_cm)}.pdf", mime="application/pdf")
            with dl_col2:
                st.download_button(label="📊 엑셀 재단표 다운로드", data=excel_bytes, file_name=f"사다리산출표_{int(L_cm)}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
